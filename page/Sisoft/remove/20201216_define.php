<?php
	...
	DEFINE('QSPAREPART2','SELECT IT.item_id Item_Code, IT.item_description Item_Name, IT.Stock Quantity,ISA.request_quantity Request, IT.remark_1 FROM invent_item_work_order ISA, invent_item IT, work_order WO WHERE ISA.itemspare=IT.item_id AND ISA.WorkOrderNo=WO.WorkOrderNo');
	...
	GROUP BY PL.PlantCode) B
	ON A.PlantId=B.PlantId');
	DEFINE('WOBACKLOG','SELECT A.WO_State, IFNULL(Total,0) Total
	FROM
	(SELECT WS.WorkStatusID, WS.WorkStatus WO_State FROM work_status WS WHERE (WS.WorkStatusID="WS000020" OR WS.WorkStatusID="WS000021" OR WS.WorkStatusID="WS000022")) A LEFT JOIN 
	(SELECT WS.WorkStatusID, WS.WorkStatus WO_State, COUNT(*) Total FROM work_order WO, work_status WS WHERE WO.WorkStatusID=WS.WorkStatusID AND WO.Hidden="no" AND YEAR(DateReceived)=YEAR(CURDATE()) AND (WS.WorkStatusID="WS000020" OR WS.WorkStatusID="WS000021" OR WS.WorkStatusID="WS000022") GROUP BY WO.WorkStatusID) B
