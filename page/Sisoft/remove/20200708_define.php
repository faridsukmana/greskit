<?php
	...
	DEFINE('WSTATE','SELECT WorkStatusID Work_Status_ID, WorkStatus Work_Status FROM work_status WHERE WorkStatusID<>"WS000001"');
	...
	WHERE 
	A.locationID=L.locationID AND A.departmentID=D.departmentID AND A.AssetStatusID=S.AssetStatusID AND A.AssetCategoryID=C.AssetCategoryID AND A.CriticalID=I.CriticalID AND A.SupplierID=P.Supplier_ID AND A.WarrantyID=W.WarrantyID AND A.EmployeeID=E.EmployeeID AND A.Hidden="no"');
	DEFINE('EDASSETS','SELECT A.AssetID Asset_ID, A.AssetNo Asset_No, A.AssetDesc Asset_Desc, A.LocationID, A.DepartmentID, A.AssetCategoryID, A.AssetStatusID, A.CriticalID, A.EmployeeID, A.SupplierID, A.Manufacturer Manufacturer, A.ModelNumber Model_Number, A.SerialNumber Serial_Number, A.WarrantyID, A.WarrantyNotes Warranty_Notes, A.WarrantyDate Warranty_Date, A.AssetNote Asset_Note, A.DateAcquired Date_Acquired, A.DateSold Date_Sold, A.ParentID FROM asset A');
	...
	DEFINE('PMCHEK','SELECT CheckListNo Check_List_No, CheckListName Check_List_Name, Task FROM pm_checklist');
	...
	DEFINE('QSPAREPART2','SELECT IT.item_id Item_Code, IT.item_description Item_Name, IT.Stock Quantity FROM invent_item_work_order ISA, invent_item IT, work_order WO WHERE ISA.itemspare=IT.item_id AND ISA.WorkOrderNo=WO.WorkOrderNo');
	DEFINE('QMANPOWER','SELECT EMP.EmployeeID ID, EMP.EmployeeNo No_ID, EMP.FirstName Name FROM work_order WO, employee EMP, work_order_manpower WM WHERE WO.WorkOrderNo=WM.WorkOrderNo AND EMP.EmployeeID=WM.EmployeeID');
	DEFINE('QICHECK','SELECT id_item_check Item_Check, description Description FROM checklist_item ');
	DEFINE('QLCHECK','SELECT id_master_checklist ID, A.AssetDesc Asset, C.description Item_Check FROM asset A, checklist_item C, checklist_master M WHERE A.AssetID=M.AssetID AND C.id_item_check=M.id_item_check');
	...
	DEFINE('GSCHEDULEWO','SELECT "Past Due Date",COUNT(*) FROM work_order WO WHERE DATE(NOW())>WO.EstDateEnd And WO.WorkStatusID<>"WS000002" AND WO.Hidden="no"
			UNION
			SELECT "Within Due Date",COUNT(*) FROM work_order WO WHERE DATE(NOW())=WO.EstDateEnd And WO.WorkStatusID<>"WS000002" AND WO.Hidden="no"
			UNION
			SELECT "Close Due Date",COUNT(*) FROM work_order WO WHERE DATE(NOW())<WO.EstDateEnd And WO.WorkStatusID<>"WS000002" AND WO.Hidden="no"');
	DEFINE('GRAPHPLNSTATE','SELECT AB.AssetStatusDesc, COUNT(AB.AssetStatusDesc) FROM asset AA, asset_status AB WHERE AA.AssetStatusID=AB.AssetStatusID GROUP BY AB.AssetStatusDesc');
	DEFINE('NOTPLANPERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000002" OR WO.WorkStatusID="WS000003" OR WO.WorkStatusID="WS000004") ORDER BY State_modified_date, WorkOrderNo DESC');
	DEFINE('PERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000007") AND YEAR(WO.ActDateEnd)=YEAR(CURRENT_DATE()) ORDER BY State_modified_date, WorkOrderNo DESC');
