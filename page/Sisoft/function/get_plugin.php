	...
	function get_data_handson($data){
		...
		while($result_now=mysql_fetch_array($result)){
			if($i<($numrow-1)){
				$content .= "{"; $j=0;
				while($j<sizeof($id)){
					$data_text = preg_replace("/\r|\n/", " ", $result_now[$j]);
					$data_text = str_replace("'", "", $data_text);
					$data_text = str_replace('"', "", $data_text);
					if($j<(sizeof($id)-1))
						$content .= $id[$j].":'".$data_text."',";
		...
		while($result_now=mysql_fetch_array($result)){ 
			if($i<($numrow-1)){
				...
				while($j<sizeof($id)){
					...
					if(strpos($data_text,'a href=')==true){
						$data_text = $data_text;
					}else{
						$data_text = str_replace('"', "", $data_text);
					}
					...
					if($j<(sizeof($id)-1))
						if(!in_array($j,$link_data))
							...
							//$content .= $id[$j].":'<span class=\"datalink\"><a href=\"".$path."&rowid=".$result_now[0]."\">".$data_text."</a></span>',";
							$content .= $id[$j].":'<span class=\"datalink\"><a href=\"".$home_path."&dataid=".$result_now[0]."#popup-article\">".$data_text."</a></span>',";
					else if($j==(sizeof($id)-1)){	
						//$content .= $id[$j].":'".$data_text."',".$func.":'<span class=\"editlink\"><a href=\"".$path."&rowid=".$result_now[0]."\">".$func."</a></span>'";
						$content .= $id[$j].":'".$data_text."',";
						$k=0;
						while($k<sizeof($func)){
							if($k<sizeof($func)-1)
