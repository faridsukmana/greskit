<?php
	...
	$id=$_POST['id'];
	$query = 'SELECT ID_Document, Document_Name, Link FROM work_document WHERE ID_Document="'.$id.'"'; 
	$result_data = mysql_exe_query(array($query,1));
	$result_data_now=mysql_exe_fetch_array(array($result_data,1));
	$location = '../../../'.$result_data_now[2]; 
	
	if(file_exists($location)){
		unlink($location);
		$info = '<tr><td colspan="5"><div class="alert alert-success" align="center"> Your file was delete</div></td></tr>';
	}else{
		$info = '<tr><td colspan="5"><div class="alert alert-success" align="center"> Your file not found in the system. May be was delete before</div></td></tr>';
	}
	
	$query = 'DELETE FROM work_document WHERE ID_Document="'.$id.'"';
