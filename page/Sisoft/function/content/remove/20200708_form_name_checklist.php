<?php
	function form_name_checklist(){
			...
			if(isset($_REQUEST['add'])){
				//***********CHECKLIST*********************		
				$query = QLCHECK.' ORDER BY A.AssetDesc, C.description ASC'; 
				
				$thead = '<thead><tr><th scope="col"><b>Asset</b></th><th scope="col"><b>Item</b></th><th scope="col"><b>Check</b></th></tr></thead>';
				$result = mysql_exe_query(array($query,1));
				$i =0; 
				while($result_now=mysql_exe_fetch_array(array($result,1))){
					$checklist .= '<tr><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" /></td></tr>';
					$i++;
				}
				$table_checklist = '<div class="col-sm-2"></div><div class="col-sm-8">'.$table_checklist.'<table class="table table-sm" style="font-size: 12px">'.$thead.'<tbody>'.$checklist.'</tbody></table></div>';
				...
				$content .= '<br/><form action="'.PATH_FORMCK.ADD.POST.'" method="post" enctype="multipart/form-data">
							<div class="form-style-1"><fieldset><legend>Form Name</legend>
								<table>
									<tr><td width="150"><span class="name"> Item Check </td><td>:</td> </span></td><td>'.text_je(array('form_name','','false')).'</td></tr>
									<tr><td></td><td></td><td><input class="form-submit" type="submit" value="Submit"></td></tr>
								</table>
							</fieldset>
							
							</div>'.$table_checklist.'</form>';
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				while($result_now=mysql_exe_fetch_array(array($result,1))){
					...
					$numrow = $rn_cform[0];
					if($numrow>0){
						$checklist .= '<tr><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" checked/></td></tr>';
					}else{
						$checklist .= '<tr><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" /></td></tr>';
					}
					$i++;
				...
				$content .= '<br/><form action="'.PATH_FORMCK.EDIT.'&rowid='.$_REQUEST['rowid'].POST.'" method="post" enctype="multipart/form-data">
							...
							<fieldset><legend>Form Name</legend>
								<table>
									<tr><td width="150"><span class="name"> Item Check </td><td>:</td> </span></td><td>'.text_je(array('form_name',$form_name,'false')).'</td></tr>
									<tr><td></td><td></td><td><input class="form-submit" type="submit" value="Submit"></td></tr>
								</table>
							</fieldset>
							</div>
