<?php
	...
	function pmgen(){
		...
		if($_REQUEST['set']=='automatic'){
			$day = (strtotime($_REQUEST['end_date'])-strtotime(date('m/d/Y')))/(60 * 60 * 24);
			if($day>21){
				$checklist .= '<tr><td colspan="6">Sorry, this function only maximum for 3 weeks. Range date is the last date until the next date, you can generate PM</td></tr>';
			}else{
	    ...
	    if(isset($_REQUEST['gen'])){
			if(isset($_REQUEST['post'])){ 
			    ...
			    while($k<$totalpm){
					...
					if($TstartDate>=$start || $TstartDate<=$end){
						if(strcmp($PMGenType,'Schedule')==0){
							if(!empty($asset)){
								$queryasset = 'SELECT A.AssetID, A.AssetDesc, L.PlantCode, L.PlantDescription FROM asset A, plant L WHERE A.PlantID=L.PlantID AND AssetID="'.$asset.'" AND (AssetStatusID="AK000011" OR AssetStatusID="AK000012")'; $asres = mysql_exe_query(array($queryasset,1)); $asres_now = mysql_exe_fetch_array(array($asres,1));
								...
								$querylocation = 'SELECT A.AssetID, A.AssetDesc, L.PlantCode, L.PlantDescription FROM asset A, plant L WHERE A.PlantID=L.PlantID AND L.PlantID="'.$location.'" AND (AssetStatusID="AK000011" OR AssetStatusID="AK000012")';  $locres = mysql_exe_query(array($querylocation,1)); 
				...
				$sethead = "['PM Name','Asset','Plant Desc','Start Date','Complete Date','Next Date']";
				//-------set id pada handson----------
