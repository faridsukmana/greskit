<?php
	function form_name_checklist(){
			...
			$sethead = "['ID Form','Form Name','Daily Checklist'"._USER_EDIT_SETHEAD_._USER_DETAIL_SETHEAD_._USER_DELETE_SETHEAD_."]";
			//-------set id pada handson----------
			$setid = "[{data:'ID',className: 'htLeft'},{data:'Form',className: 'htLeft'},{data:'Daily',className: 'htLeft'}"._USER_EDIT_SETID_._USER_DETAIL_SETID_._USER_DELETE_SETID_."]";
			//-------get data pada sql------------
			...
			if(isset($_REQUEST['add'])){
				...
				while($result_now=mysql_exe_fetch_array(array($result,1))){
					$checklist .= '<tr><td>'.$i.'</td><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" onclick="check_list(\''.$result_now[0].'\');" /></td></tr>';
					$i++;
				}
				$table_checklist = '<div class="col-sm-2"></div><div class="col-sm-8">'.$table_checklist.'<!--<table id="tb_form" class="table table-bordered table-striped" style="font-size: 12px">'.$thead.'<tbody>'.$checklist.'</tbody></table></div>-->';
			...
			if(isset($_REQUEST['edit'])){ $info='';
				if(isset($_REQUEST['post'])){
					if(!empty($_REQUEST['form_name'])){
						...
						$query = 'UPDATE checklist_form_name SET form_name="'.$_REQUEST['form_name'].'", daily_checklist="'.$_REQUEST['dlcheck'].'" WHERE id_form_checklist="'.$_REQUEST['rowid'].'"'; mysql_exe_query(array($query,1));  
						...
						$info .= '<div class="alert alert-success" align="center" role="alert">Name form was changed</div>';
					}else{
						//$info .= '<br/><div id="example1" style="width: 1335px; height: 80px; overflow: hidden; font-size=10px;"></div>';
						$info .= '<div class="alert alert-danger" align="center" role="alert">Name form failed changed, some field is empty</div>';
					}
				}
				...
				while($result_now=mysql_exe_fetch_array(array($result,1))){
					...
					$numrow = $rn_cform[0];
					if($numrow>0){
						$checklist .= '<tr><td>'.$i.'</td><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input id="'.$_REQUEST['rowid'].'_'.$result_now[0].'" type="checkbox" name="clist_'.$result_now[0].'" value="v" onclick="check_list(\''.$_REQUEST['rowid'].'\',\''.$result_now[0].'\');" checked/></td></tr>';
					}else{
						$checklist .= '<tr><td>'.$i.'</td><td>'.$result_now[1].'</td><td width="300">'.$result_now[2].'</td><td><input id="'.$_REQUEST['rowid'].'_'.$result_now[0].'" type="checkbox" name="clist_'.$result_now[0].'" value="v" onclick="check_list(\''.$_REQUEST['rowid'].'\',\''.$result_now[0].'\');" /></td></tr>';
					}
					$i++;
				}
				$table_checklist = '<div class="col-sm-2"></div><div class="col-sm-8">'.$table_checklist.'<table id="tb_form" class="table table-bordered table-striped" style="font-size: 12px">'.$thead.'<tbody>'.$checklist.'</tbody></table></div>';
				...
				$content .= '<br/><form action="'.PATH_FORMCK.EDIT.'&rowid='.$_REQUEST['rowid'].POST.'" method="post" enctype="multipart/form-data">
							...
							<fieldset><div class="card-header text-center">Form Name</div>
								<table>
									...
									<td width="150"><span class="name">Daily Checklist</td><td>:</td><td>'.combo_je_arr(array(array('yes','no'),'dlcheck','dlcheck',225,'',$dlcheck)).'</td>
									<tr><td></td><td></td><td><input class="form-submit" type="submit" value="Change Form Name"></td></tr>
								</table>
							</fieldset>
							</div>
	...
	function form_js(){
		$content = "
					<script>
						...
						function check_list(id_form,id_master){
							if($('#'+id_form+'_'+id_master).is(':checked')){
								$.ajax({
									type: 'POST',
									url:'"._ROOT_."function/content/checklist/edit_checklist.php',
									data:{'id_form':id_form,'id_master':id_master},
									crossDomain:true,
									cache:false,
									success:function(data){
										alert(data+' input successed');
									}
								})
							}
							
							else if($('#'+id_form+'_'+id_master).is(':not(:checked)')){
								$.ajax({
									type: 'POST',
									url:'"._ROOT_."function/content/checklist/delete_checklist.php',
									data:{'id_form':id_form,'id_master':id_master},
									crossDomain:true,
									cache:false,
									success:function(data){
										alert(data+' delete successed');
									}
								})
							}
						}
