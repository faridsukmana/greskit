	...
	function create_daily_checklist(){
		...
		if(isset($_REQUEST['add'])){
			...
			if(isset($_REQUEST['post'])){
				if(!empty($_REQUEST['form']) && !empty($_REQUEST['dateh'])){
					...
					$query_c = 'SELECT A.id_form_checklist, A.id_master_checklist, C.default_val FROM checklist_form_master A, checklist_master B, checklist_item C WHERE A.id_master_checklist=B.id_master_checklist AND B.id_item_check=C.id_item_check AND A.id_form_checklist="'.$_REQUEST['form'].'"';
					$result_c = mysql_exe_query(array($query_c,1));
					while($result_now_c=mysql_exe_fetch_array(array($result_c,1))){
						$query = 'INSERT INTO checklist_history (id_checklist_history ,date, id_form_checklist, id_master_checklist, description) VALUES("'.$wpid.'","'.$dateh.'","'.$result_now_c[0].'","'.$result_now_c[1].'","'.$result_now_c[2].'")'; 
						mysql_exe_query(array($query,1));
					}
		...
		if(isset($_REQUEST['edit'])){ $info='';
			...
			while($result_now=mysql_exe_fetch_array(array($result,1))){
				...
				$checklist .= '<tr><td>'.$result_now[1].'</td><td>'.$result_now[2].'</td><td width="500"><input class="form-control" type="text" name="text_'.$result_now[4].'" value="'.$def.'" /></td></tr>';
				$i++;
			}
