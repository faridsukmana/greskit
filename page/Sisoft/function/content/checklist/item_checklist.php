<?php
	function item_checklist(){
		/**----- Delete -------------------*/
			...
			$sethead = "['Item Check ID','Item Check','Default Value'"._USER_EDIT_SETHEAD_._USER_DELETE_SETHEAD_."]";
			//-------set id pada handson----------
			$setid = "[{data:'Item_Check',className: 'htLeft'},{data:'Description',className: 'htLeft'},{data:'Default_Val',className: 'htLeft'}"._USER_EDIT_SETID_._USER_DELETE_SETID_."]";
			//-------get data pada sql------------
			...
			if(isset($_REQUEST['add'])){
				$content = '<br/><div class="ade">ADD DATA FOR ITEM CHECKLIST</div>';
				$content .= '<div class="toptext" align="center">'._USER_VIEW_._USER_INSERT_.'</div>';
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_ICHECK.ADD.POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Item Checklist</div>
								<table>
									<tr><td width="150"><span class="name"> Item Check </td><td>:</td> </span></td><td>'.text_je(array('icheck','','false')).'</td></tr>
									<tr><td width="150"><span class="name"> Default Value </td><td>:</td> </span></td><td>'.text_je(array('defval','','false')).'</td></tr>
				...
				if(isset($_REQUEST['post'])){
					if(!empty($_REQUEST['icheck'])){
						...
						$query = 'INSERT INTO checklist_item (id_item_check ,description,default_val) VALUES("'.$wpid.'","'.$_REQUEST['icheck'].'","'.$_REQUEST['defval'].'")'; mysql_exe_query(array($query,1));
						...
						$sethandson = array($sethead,$setid,$data,$width,$fixedcolleft);
						$content .= get_handson($sethandson);
						$content .= '<div class="alert alert-success" align="center" role="alert">Insert Successed</div>';
					}else{
						$content .= '<div class="alert alert-danger" align="center" role="alert">Insert Failed, some field is empty</div>';
					}
				}
			}
			...
			if(isset($_REQUEST['edit'])){ $info='';
				if(isset($_REQUEST['post'])){
					if(!empty($_REQUEST['icheck'])){
						...
						$query = 'UPDATE checklist_item SET description="'.$_REQUEST['icheck'].'", default_val="'.$_REQUEST['defval'].'" WHERE id_item_check="'.$_REQUEST['rowid'].'"'; mysql_exe_query(array($query,1));  
						//-- Ambil data baru dari database --//
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_ICHECK.EDIT.'&rowid='.$_REQUEST['rowid'].POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Item Checklist</div>
								<table>
									...
									<tr><td width="150"><span class="name"> Default Value </td><td>:</td> </span></td><td>'.text_je(array('defval',$defval,'false')).'</td></tr>
									<tr><td></td><td></td><td><input class="form-submit" type="submit" value="Submit"></td></tr>
								</table>
							</fieldset>
							</form></div>';
