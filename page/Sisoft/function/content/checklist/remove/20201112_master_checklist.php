<?php
	function master_checklist(){
			...
			if(isset($_REQUEST['add'])){
				...
				$thead = '<thead><tr><th scope="col"><b>Item Check</b></th><th scope="col"><b>Check</b></th></tr></thead>';
				$result = mysql_exe_query(array($query,1));
				$i =0; 
				while($result_now=mysql_exe_fetch_array(array($result,1))){
					$checklist .= '<tr><td>'.$result_now[1].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" /></td></tr>';
					$i++;
				}
				...
				$content .= '<br/><form action="'.PATH_LCHECK.ADD.POST.'" method="post" enctype="multipart/form-data">
							<div class="form-style-1"><fieldset><div class="card-header text-center">Work Priority</div>
								<table>
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				while($result_now=mysql_exe_fetch_array(array($result,1))){ 
					...
					$numrow = $rn_cform[0];
					if($numrow>0){
						$checklist .= '<tr><td>'.$result_now[1].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" checked/></td></tr>';
					}else{
						$checklist .= '<tr><td>'.$result_now[1].'</td><td><input type="checkbox" name="clist_'.$result_now[0].'" value="v" /></td></tr>';
					}
					$i++;
