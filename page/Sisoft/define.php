<?php
	...
	DEFINE('PMSCHE','SELECT PS.PM_ID, AE.AssetDesc Asset, LC.PlantCode Plant_Code, WT.WorkTrade Work_Trade, PS.PMName PM_Name, PC.CheckListName PM_Task, PS.PMState PM_State, PS.FreqUnits Frequency_Unit, PS.InitiateDate Initiate_Date, PS.TargetStartDate Target_Start_Date, PS.TargetCompDate Target_Comp_Date, PS.NextDate Next_Date FROM pm_schedule PS LEFT JOIN asset AE ON PS.AssetID=AE.AssetID LEFT JOIN plant LC ON PS.locationID=LC.PlantID, work_trade WT, pm_checklist PC WHERE PS.PMWOTrade=WT.WorkTradeID AND PS.ChecklistNo=PC.CheckListNo AND PS.Hidden="no"');
	...
	DEFINE('QICHECK','SELECT id_item_check Item_Check, description Description, default_val Default_Val FROM checklist_item ');
	...
	DEFINE('QFORMCK','SELECT id_form_checklist ID, form_name Form, daily_checklist Daily FROM checklist_form_name');
	...
	if($_SESSION['groupID']=='GROUP181120033150'){
	...
	}else if($_SESSION['groupID']=='GROUP181120025602'){
		if(isset($_REQUEST['rowid'])){
			DEFINE('COMWOSTAT','
			SELECT WorkStatusID, WorkStatus FROM work_status WHERE WorkStatusID IN (SELECT WorkStatusID FROM work_order WHERE WorkOrderNo="'.$_REQUEST['rowid'].'")
			UNION
		...
		if(isset($_REQUEST['rowid'])){
			...
			SELECT WorkStatusID, WorkStatus FROM work_status WHERE WorkStatusID IN (SELECT WorkStatusID FROM work_order WHERE WorkOrderNo="'.$_REQUEST['rowid'].'")
			UNION
			SELECT WorkStatusID, WorkStatus FROM work_status where id_group ="'.$_SESSION['groupID'].'" OR id_group =""');
		}else{
	...
	//----------Dashboard Timuraya -------------------------------------//
	DEFINE('HISBREAKDOWN','SELECT PL.PlantCode Plant, AA.AssetNo, DATE(WO.ActDateStart) Start_Date, DATE(WO.ActDateEnd) Finish_Date, IFNULL(DATEDIFF(WO.ActDateEnd,WO.ActDateStart),0) Days, TIME(WO.ActDateStart) Start_Time, TIME(WO.ActDateEnd) Finish_Time, IFNULL(TIMESTAMPDIFF(HOUR, WO.ActDateStart, WO.ActDateEnd),0) Hour_, WS.WorkStatus Status_  FROM work_order WO, plant PL, asset AA, work_status WS WHERE WO.AssetID=AA.AssetID AND AA.PlantId=PL.PlantId AND WO.WorkStatusID=WS.WorkStatusID AND WO.WorkTypeID="WT000001"
	ORDER BY WO.WorkOrderNo DESC');
	DEFINE('CALCULATION','
	SELECT PL.PlantCode, AA.AssetID, AA.AssetNo, IFNULL(FR.Freq,0) Freq, IFNULL(DD.Down_days,0) Down_Days, IFNULL(DD.Down_Hours,0) Down_Hours, IFNULL(MT.Down_days/IFNULL(FR.Freq,1),0) MT_Days, IFNULL(MT.Down_Hours/IFNULL(FR.Freq,1),0) MT_Hours, IFNULL(MB.TDay,0) MB_Days, IFNULL(MB.THours,0) MB_Hours FROM plant PL, asset AA LEFT JOIN
	(SELECT AssetID, COUNT(*) Freq FROM work_order WHERE WorkTypeID="WT000001" GROUP BY AssetID) FR ON AA.AssetID=FR.AssetID LEFT JOIN
	(SELECT WO.AssetID, WO.WorkOrderNo, SUM(DATEDIFF(WO.ActDateEnd,WO.ActDateStart)) Down_days, SUM(TIMESTAMPDIFF(HOUR, WO.ActDateStart, WO.ActDateEnd)) Down_Hours FROM work_order WO WHERE WO.WorkTypeID="WT000014" AND WO.Hidden="no" GROUP BY WO.AssetID) DD ON AA.AssetID=DD.AssetID LEFT JOIN
	(SELECT WO.AssetID, WO.WorkOrderNo, SUM(DATEDIFF(WO.ActDateEnd,WO.ActDateStart)) Down_days, SUM(TIMESTAMPDIFF(HOUR, WO.ActDateStart, WO.ActDateEnd)) Down_Hours FROM work_order WO WHERE WO.WorkTypeID="WT000001" AND WO.Hidden="no" GROUP BY WO.AssetID) MT ON AA.AssetID=MT.AssetID LEFT JOIN
	(SELECT TOTAL.AssetID, ROUND(SUM(Days)/(COUNT(*)+1),0) TDay, ROUND(SUM(Hours)/(COUNT(*)+1),0) THours FROM
	(SELECT X.WorkOrderNo, X.AssetID, X.PlantCode, X.ActDateEnd, Y.ActDateStart, DATEDIFF(Y.ActDateEnd,X.ActDateStart) Days, TIMESTAMPDIFF(HOUR, X.ActDateStart, Y.ActDateEnd) Hours
	FROM
	(SELECT (@cnt := @cnt + 1) No_, A.WorkOrderNo, A.AssetID, A.PlantCode, A.ActDateStart, A.ActDateEnd
	FROM
	(SELECT WO.WorkOrderNo, AE.AssetID, PL.PlantCode, WO.ActDateStart, WO.ActDateEnd FROM work_order WO, asset AE, plant PL
	WHERE PL.PlantID=AE.PlantID AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND WO.Hidden="no" ORDER BY AE.AssetID,WorkOrderNo LIMIT 999999) A cross join (select @cnt := 0) r) X,

	(SELECT (@cnt2 := @cnt2 + 1) No_, D.WorkOrderNo, D.AssetID, D.PlantCode, D.ActDateStart, D.ActDateEnd
	FROM
	(SELECT WO.WorkOrderNo, AE.AssetID, PL.PlantCode, ActDateStart, ActDateEnd FROM work_order WO, asset AE, plant PL, 
	(SELECT WorkOrderNo FROM work_order WO, asset AE, plant PL WHERE PL.PlantID=AE.PlantID AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND WO.Hidden="no" ORDER BY AE.AssetID,WorkOrderNo LIMIT 1) S 
	WHERE PL.PlantID=AE.PlantID AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND WO.Hidden="no" AND WO.WorkOrderNo<>S.WorkOrderNo ORDER BY AE.AssetID,WorkOrderNo LIMIT 999999) D cross join (select @cnt2 := 0) r) Y

	WHERE X.No_=Y.No_ AND X.AssetID=Y.AssetID) TOTAL GROUP BY TOTAL.AssetID) MB ON AA.AssetID=MB.AssetID
	WHERE PL.PlantID=AA.PlantID 
	');
	DEFINE('ALL_PLANT_MTBF_HOUR','SELECT PLANT.PlantCode, IFNULL(HRTOTAL.THour,0) FROM
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) PLANT LEFT JOIN 
	(SELECT TOTAL.PlantCode, SUM(Hours)/(COUNT(*)+1) THour FROM
	(SELECT X.WorkOrderNo, X.AssetID, X.PlantCode, X.ActDateEnd, Y.ActDateStart, TIMESTAMPDIFF(HOUR, X.ActDateStart, Y.ActDateEnd) Hours
	FROM
	(SELECT (@cnt := @cnt + 1) No_, A.WorkOrderNo, A.AssetID, A.PlantCode, A.ActDateStart, A.ActDateEnd
	FROM
	(SELECT WO.WorkOrderNo, AE.AssetID, PL.PlantCode, WO.ActDateStart, WO.ActDateEnd FROM work_order WO, asset AE, plant PL
	...
	WHERE X.No_=Y.No_ AND X.AssetID=Y.AssetID) TOTAL GROUP BY TOTAL.PlantCode) HRTOTAL ON
	PLANT.PlantCode=HRTOTAL.PlantCode
	');
	DEFINE('ALL_PLANT_MTBF_DAY','SELECT PLANT.PlantCode, IFNULL(HRTOTAL.TDay,0) FROM
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) PLANT LEFT JOIN 
	(SELECT TOTAL.PlantCode, ROUND(SUM(Days)/(COUNT(*)+1),0) TDay FROM
	(SELECT X.WorkOrderNo, X.AssetID, X.PlantCode, X.ActDateEnd, Y.ActDateStart, DATEDIFF(Y.ActDateEnd,X.ActDateStart) Days
	FROM
	(SELECT (@cnt := @cnt + 1) No_, A.WorkOrderNo, A.AssetID, A.PlantCode, A.ActDateStart, A.ActDateEnd
	FROM
	(SELECT WO.WorkOrderNo, AE.AssetID, PL.PlantCode, WO.ActDateStart, WO.ActDateEnd FROM work_order WO, asset AE, plant PL
	...
	');
	DEFINE('ALL_PLANT_MTTR_HOUR','SELECT A.PLantCode, IFNULL(B.MTTR,0)
	FROM 
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) A LEFT JOIN
	(SELECT PL.PlantId, PL.PlantCode, SUM(TIMESTAMPDIFF(HOUR, ActDateStart, ActDateEnd)) hours, COUNT(PL.PlantCode) Count, (SUM(TIMESTAMPDIFF(HOUR, ActDateStart, ActDateEnd))/COUNT(PL.PlantCode)) MTTR FROM work_order WO, asset AE, plant PL WHERE PL.PlantId=AE.PlantId AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND  WO.Hidden="no" 
	GROUP BY PL.PlantCode) B
	ON A.PlantId=B.PlantId');
	DEFINE('ALL_PLANT_MTTR_DAY','SELECT A.PLantCode, ROUND(IFNULL(B.MTTR,0),0)
	FROM 
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) A LEFT JOIN
	(SELECT PL.PlantId, PL.PlantCode, SUM(DATEDIFF(ActDateEnd,ActDateStart)) hours, COUNT(PL.PlantCode) Count, (SUM(DATEDIFF(ActDateEnd,ActDateStart))/COUNT(PL.PlantCode)) MTTR FROM work_order WO, asset AE, plant PL WHERE PL.PlantId=AE.PlantId AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND  WO.Hidden="no" 
	GROUP BY PL.PlantCode) B
	ON A.PlantId=B.PlantId');
	...
	DEFINE('WAITAPPR','SELECT WorkOrderNo WO, P.PlantCode Plant_Code, AssetNo Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E, plant P WHERE P.PlantId=A.PlantId AND A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000014") ORDER BY WorkOrderNo DESC');
	DEFINE('OPEN','SELECT WorkOrderNo WO, P.PlantCode Plant_Code, AssetNo Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E, plant P WHERE P.PlantId=A.PlantId AND A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000022") ORDER BY WorkOrderNo DESC');
	DEFINE('CLOSE','SELECT WorkOrderNo WO, P.PlantCode Plant_Code, AssetNo Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E, plant P WHERE P.PlantId=A.PlantId AND A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000020") ORDER BY WorkOrderNo DESC');
	DEFINE('WAITEXEC','SELECT WorkOrderNo WO, P.PlantCode Plant_Code, AssetNo Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E, plant P WHERE P.PlantId=A.PlantId AND A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000010" OR WO.WorkStatusID="WS000012" OR WO.WorkStatusID="WS000013" OR WO.WorkStatusID="WS000019") ORDER BY WorkOrderNo DESC');
	DEFINE('WAITEXECMGR','SELECT WorkOrderNo WO, P.PlantCode Plant_Code, AssetNo Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E, plant P WHERE P.PlantId=A.PlantId AND A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND WO.WorkStatusID NOT IN (SELECT WorkStatusID FROM work_status WHERE WorkStatusID="WS000020" OR WorkStatusID="WS000021") ORDER BY WorkOrderNo DESC');
	
	DEFINE('ALL_PLANT_MTBF_DAY_KR','SELECT PLANT.PlantCode, IFNULL(HRTOTAL.TDay,0) FROM
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) PLANT LEFT JOIN 
	(SELECT TOTAL.PlantCode, ROUND(SUM(Days)/(COUNT(*)+1),0) TDay FROM
	(SELECT X.WorkOrderNo, X.AssetID, X.PlantCode, X.ActDateEnd, Y.ActDateStart, DATEDIFF(Y.ActDateEnd,X.ActDateStart) Days
	FROM
	...
	WHERE X.No_=Y.No_ AND X.AssetID=Y.AssetID) TOTAL GROUP BY TOTAL.PlantCode) HRTOTAL ON
	PLANT.PlantCode=HRTOTAL.PlantCode
	WHERE Plant.PlantCode IN ("KR-AS","KR-SA-1","KR-SA-2","KR-ZK-1","KR-ZK-2","KR-ZA","KR-AP","KR-AC")
	');
	
	DEFINE('ALL_PLANT_MTBF_DAY_TG','SELECT PLANT.PlantCode, IFNULL(HRTOTAL.TDay,0) FROM
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) PLANT LEFT JOIN 
	(SELECT TOTAL.PlantCode, ROUND(SUM(Days)/(COUNT(*)+1),0) TDay FROM
	(SELECT X.WorkOrderNo, X.AssetID, X.PlantCode, X.ActDateEnd, Y.ActDateStart, DATEDIFF(Y.ActDateEnd,X.ActDateStart) Days
	FROM
	...
	WHERE X.No_=Y.No_ AND X.AssetID=Y.AssetID) TOTAL GROUP BY TOTAL.PlantCode) HRTOTAL ON
	PLANT.PlantCode=HRTOTAL.PlantCode
	WHERE Plant.PlantCode IN ("TG-AS","TG-SA")
	');
	
	DEFINE('ALL_PLANT_MTTR_DAY_KR','SELECT A.PLantCode, ROUND(IFNULL(B.MTTR,0),0)
	FROM 
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) A LEFT JOIN
	(SELECT PL.PlantId, PL.PlantCode, SUM(DATEDIFF(ActDateEnd,ActDateStart)) hours, COUNT(PL.PlantCode) Count, (SUM(DATEDIFF(ActDateEnd,ActDateStart))/COUNT(PL.PlantCode)) MTTR FROM work_order WO, asset AE, plant PL WHERE PL.PlantId=AE.PlantId AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND  WO.Hidden="no" 
	GROUP BY PL.PlantCode) B
	ON A.PlantId=B.PlantId
	WHERE A.PlantCode IN ("KR-AS","KR-SA-1","KR-SA-2","KR-ZK-1","KR-ZK-2","KR-ZA","KR-AP","KR-AC")');
	
	DEFINE('ALL_PLANT_MTTR_DAY_TG','SELECT A.PLantCode, ROUND(IFNULL(B.MTTR,0),0)
	FROM 
	(SELECT PL.PlantId, PL.PlantCode FROM plant PL) A LEFT JOIN
	(SELECT PL.PlantId, PL.PlantCode, SUM(DATEDIFF(ActDateEnd,ActDateStart)) hours, COUNT(PL.PlantCode) Count, (SUM(DATEDIFF(ActDateEnd,ActDateStart))/COUNT(PL.PlantCode)) MTTR FROM work_order WO, asset AE, plant PL WHERE PL.PlantId=AE.PlantId AND AE.AssetID=WO.AssetID AND WO.WorkStatusID="WS000020" AND WO.WorkTypeID="WT000001" AND  WO.Hidden="no" 
	GROUP BY PL.PlantCode) B
	ON A.PlantId=B.PlantId
	WHERE A.PlantCode IN ("TG-AS","TG-SA")');
	...
	DEFINE('NOTPLANPERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000022") ORDER BY WorkOrderNo DESC');
	DEFINE('PERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID<>"WS000020" OR WO.WorkStatusID<>"WS000021" OR WO.WorkStatusID<>"WS000022" OR WO.WorkStatusID<>"WS000001") AND YEAR(WO.ActDateEnd)=YEAR(CURRENT_DATE()) ORDER BY WorkOrderNo DESC');
