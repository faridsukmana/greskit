...
########################################################## KHUSUS HALAMAN WEB#############################################################
	...
	if(_ACCESS_){	
		...
		//------ Account --------------
			...
			//-------set lebar kolom -------------
			$width = "[100,100,150,150,150,200,80,100]";
			//-------get id pada sql -------------
			$field = gen_mysql_id(USER.' ORDER BY id_user DESC');
			//-------get header pada sql----------
			$name = gen_mysql_head(USER.' ORDER BY id_user DESC');
			//-------set header pada handson------
			$sethead = "['User ID','Username','First_name','Nick Name','Last Name','Email','Modified','Password']";
			//-------set id pada handson----------
			$setid = "[{data:'User_ID',className: 'htLeft'},{data:'Username',className: 'htLeft'},{data:'First_Name',className: 'htLeft'},{data:'Nick_Name',className: 'htLeft'},{data:'Last_Name',className: 'htLeft'},{data:'Email',className: 'htLeft'},{data:'Edit',renderer: 'html'},{data:'Change',renderer: 'html'}]";
			//-------get data pada sql------------
			...
			if(isset($_REQUEST['add'])){
				...
				$content .= '<div class="" align="center"><span><a class="btn btn-outline-primary btn-rounded btn-fw" href="'.PATH_USER.'">View</a></span> <span><a class="btn btn-outline-primary btn-rounded btn-fw" href="'.PATH_USER.ADD.'">Add</a></span></div>';
				//----- Query employee -------------
				$query_employee = 'SELECT id_employee, employee_name FROM sms_re_identity_employee';
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				$querydat = USER.' WHERE id_user="'.$_REQUEST['rowid'].'"'; $result=mysql_exe_query(array($querydat,1)); $resultnow=mysql_exe_fetch_array(array($result,1));
				$username=$resultnow[1]; $password=$resultnow[6]; $firstname=$resultnow[2]; $nickname=$resultnow[3]; $lastname=$resultnow[4]; $email=$resultnow[5];
				...
				//----- Buat Form Isian Berikut-----
				$name_field=array('First Name','Nick Name','Last Name','Email','Username');
				$input_type=array(
