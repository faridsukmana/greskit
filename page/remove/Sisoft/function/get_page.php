...
########################################################## KHUSUS HALAMAN WEB#############################################################
	...
	if(_ACCESS_){	
		...
		//========Halaman contoh penggunaan Highchart=======
			...
			$content .= get_js_graph(array(GRAPHPLNSTATE,'3d-pie','Work Load Precentage','Percent Asset','container2','300','360','typequery1',PATH_INDEX_PAGE,'Total Asset')); 
			...
			while($result_now=mysql_exe_fetch_array(array($result,1))){
				$detail_1 .= '<button type="button" class="btn badge-secondary" style="margin-top:3px;">
							  '.$result_now[0].' <span class="badge badge-info">'.$result_now[1].'</span>
							</button> ';
				$i++;
			}
			
			$query = GRAPHPLNSTATE; $detail='';
			$result = mysql_exe_query(array($query,1)); 
			$i=0;
			while($result_now=mysql_exe_fetch_array(array($result,1))){
				$detail_2 .= '<button type="button" class="btn badge-secondary" style="margin-top:3px;">
							  '.$result_now[0].' <span class="badge badge-info">'.$result_now[1].'</span>
							</button> ';
				$i++;
			}
			
			$query = GSCHEDULEWO; $detail='';
			$result = mysql_exe_query(array($query,1)); 
			$i=0;
			while($result_now=mysql_exe_fetch_array(array($result,1))){
				$detail_3 .= '<button type="button" class="btn badge-secondary" style="margin-top:3px;">
							  '.$result_now[0].' <span class="badge badge-info">'.$result_now[1].'</span>
							</button> ';
				$i++;
			}
				...
				<div class="col-lg-4">
					<div class="panel panel-default">
                        ...
                        <div class="panel-body">
							...
							<div class="text-center">'.$detail_1.'</div>
                        </div>
                        <!-- /.panel-body -->
                    </div>
                    <!-- /.panel -->
				...
				<div class="col-lg-4">
					<div class="panel panel-default">
                        ...
                        <div class="panel-body">
							...
							<div class="text-center">'.$detail_2.'</div>
                        </div>
                        <!-- /.panel-body -->
                    </div>
                    <!-- /.panel -->
				...
				<div class="col-lg-4">
					<div class="panel panel-default">
                        ...
                        <div class="panel-body">
							...
							<div class="text-center">'.$detail_3.'</div>
                        </div>
                        <!-- /.panel-body -->
                    </div>
                    <!-- /.panel -->
		...
		//------ EMPLOYEE ------------------
			...
			$content .= '<br/><div id="example1" style="width: 1030px; height: 500px; overflow: hidden; font-size=10px;"></div>';
			//-------set lebar kolom -------------
			$width = "[150,150,150,150,150,300,250,150,150,150,150,150,150,150,150,80,80]";
			//-------get id pada sql -------------
		...
		//------ DEPARTMENT ------------------
			...
			if(isset($_REQUEST['add'])){
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_DEPART.ADD.POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Department</div>
								<table>
		...
		//------ LOCATION ------------------
			...
			//-------set header pada handson------
			$sethead = "['Process Unit ID','Process Unit No','Process Unit Description','Department Desc','Process Unit Information','Remark 1','Remark 2','Remark 3'"._USER_EDIT_SETHEAD_._USER_DELETE_SETHEAD_."]";
			//-------set id pada handson----------
			...
			if(isset($_REQUEST['add'])){
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_LOCATN.ADD.POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Process Unit</div>
								<table>
									<tr><td width="150"><span class="name"> Process Unit No </td><td>:</td> </span></td><td>'.text_je(array('locno','','false')).' *</td></tr>
				...
				if(isset($_REQUEST['post'])){
					if(!empty($_REQUEST['locno']) && !empty($_REQUEST['locdes']) && !empty($_REQUEST['depdes'])){
						...
						$name = gen_mysql_head($querydat);
						//-------set header pada handson------
						$sethead = "['Process Unit ID','Process Unit No','Process Unit Description','Department Desc','Process Unit Information','Remark 1','Remark 2','Remark 3'"._USER_EDIT_SETHEAD_."]";
						//-------set id pada handson----------
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_LOCATN.EDIT.'&rowid='.$_REQUEST['rowid'].POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Process Unit</div>
								<table>
									<tr><td width="150"><span class="name"> Process Unit No </td><td>:</td> </span></td><td>'.text_je(array('locno',$locno,'false')).' *</td></tr>
		...
		//------ ASSET CRITICALLY --------------
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				$content .= '<br/><div class="form-style-1"><form action="'.PATH_WTRADE.EDIT.'&rowid='.$_REQUEST['rowid'].POST.'" method="post" enctype="multipart/form-data">
							<fieldset><div class="card-header text-center">Work Trade</div>
								<table>
