<?php
	function asset(){
			...
			$content .= '<br/><div class="ade">ASSETS TES</div>';
			...
			//-------set lebar kolom -------------
			if(_DELETE_) $width = "[150,150,200,80,80,100,100,200,200,200,150,150,150,200,200,200,200,200,150,150,150]";
			...
			if ($_SESSION['userID'] !='') {
				...
				$sethead = "['Asset ID','Asset No','Asset Desc'"._USER_EDIT_SETHEAD_._USER_DELETE_SETHEAD_._USER_HISTORY_SETHEAD_._USER_DOC_SETHEAD_.",'Location Desc','Department Desc','Asset Category','Asset Status','Critically', 'Auth Employeee','Supplier Name','Manufacturer','Model Number','Serial Number','Warranty', 'Warranty Date','Date Acquired','Date Sold']";
				//-------set id pada handson----------
				$setid = "[{data:'Asset_ID',renderer: 'html',className: 'htLeft'},{data:'Asset_No',className: 'htLeft'},{data:'Asset_Desc',className: 'htLeft'}"._USER_EDIT_SETID_._USER_DELETE_SETID_._USER_HISTORY_SETID_._USER_DOC_SETID_.",{data:'Location_Desc',className: 'htLeft'},{data:'Department_Desc',className: 'htLeft'},{data:'Asset_Category',className: 'htLeft'},{data:'Asset_Status',className: 'htLeft'},{data:'Critically',className: 'htLeft'},{data:'Auth_Employee',className: 'htLeft'},{data:'Supplier_Name',className: 'htLeft'},{data:'Manufacturer',className: 'htLeft'},{data:'Model_Number',className: 'htLeft'},{data:'Serial_Number',className: 'htLeft'},{data:'Warranty',className: 'htLeft'},{data:'Warranty_Date',type: 'date',dateFormat: 'YYYY-MM-DD'},{data:'Date_Acquired',type: 'date',dateFormat: 'YYYY-MM-DD'},{data:'Date_Sold',type: 'date',dateFormat: 'YYYY-MM-DD'}]";
				//-------get data pada sql------------
				$dt = array(ASSETS.' ORDER BY A.AssetID DESC',$field,array('Edit','Delete','History','Attachment'),array(PATH_ASSETS.EDIT,PATH_ASSETS.DELETE,PATH_ASSETS.HISTORY,PATH_ASSETS.DOCUMENT),array(0),PATH_ASSETS);
				...
				$field = gen_mysql_id(ASSETSNOID.' ORDER BY A.AssetID DESC');
				//-------get header pada sql----------
				$name = gen_mysql_head(ASSETSNOID.' ORDER BY A.AssetID DESC');
				$sethead = "['Asset ID','Asset No','Asset Desc'"._USER_EDIT_SETHEAD_._USER_DELETE_SETHEAD_._USER_HISTORY_SETHEAD_._USER_DOC_SETHEAD_.",'Location Desc','Department Desc','Asset Category','Asset Status','Critically', 'Auth Employeee','Supplier Name','Manufacturer','Model Number','Serial Number','Warranty', 'Warranty Date','Date Acquired','Date Sold']";
				//-------set id pada handson----------
				$setid = "[{data:'Asset_ID',renderer: 'html',className: 'htLeft'},{data:'Asset_No',className: 'htLeft'},{data:'Asset_Desc',className: 'htLeft'}"._USER_EDIT_SETID_._USER_DELETE_SETID_._USER_HISTORY_SETID_._USER_DOC_SETID_.",{data:'Location_Desc',className: 'htLeft'},{data:'Department_Desc',className: 'htLeft'},{data:'Asset_Category',className: 'htLeft'},{data:'Asset_Status',className: 'htLeft'},{data:'Critically',className: 'htLeft'},{data:'Auth_Employee',className: 'htLeft'},{data:'Supplier_Name',className: 'htLeft'},{data:'Manufacturer',className: 'htLeft'},{data:'Model_Number',className: 'htLeft'},{data:'Serial_Number',className: 'htLeft'},{data:'Warranty',className: 'htLeft'},{data:'Warranty_Date',type: 'date',dateFormat: 'YYYY-MM-DD'},{data:'Date_Acquired',type: 'date',dateFormat: 'YYYY-MM-DD'},{data:'Date_Sold',type: 'date',dateFormat: 'YYYY-MM-DD'}]";
				//-------get data pada sql------------
				$dt = array(ASSETSNOID.' ORDER BY A.AssetID DESC',$field,array('Edit','Delete','History','Attachment'),array(PATH_ASSETS.EDIT,PATH_ASSETS.DELETE,PATH_ASSETS.HISTORY,PATH_ASSETS.DOCUMENT),array(0),PATH_ASSETS);
				$data = get_data_handson_func($dt);
				//----Fungsi memanggil data handsontable melalui javascript---
				$fixedcolleft=3;
				$sethandson = array($sethead,$setid,$data,$width,$fixedcolleft);
	...
	function pop_detail_report(){
		...
		//***********MTBF AND MTTF***************//
		$query_data = 'SELECT WorkOrderNo, ActDateStart, ActDateEnd FROM work_order WO WHERE WO.AssetID="'.$_REQUEST['dataid'].'" AND WO.WorkStatusID="WS000010" AND WO.Hidden="no"';
		$result_data = mysql_exe_query(array($query_data,1));
		$i=0; $sum_wo = 0;
		while($result_data_now=mysql_exe_fetch_array(array($result_data,1))){
		...
		while($i<sizeof($datetime)-1){
			...
			$diff = $second-$first;//echo $diff.'<br/>';
