<?php
	...
	WHERE 
	A.locationID=L.locationID AND A.departmentID=D.departmentID AND A.AssetStatusID=S.AssetStatusID AND A.AssetCategoryID=C.AssetCategoryID AND A.CriticalID=I.CriticalID AND A.SupplierID=P.Supplier_ID AND A.WarrantyID=W.WarrantyID AND A.EmployeeID=E.EmployeeID AND A.Hidden='no' AND A.departmentID ='{$_SESSION['section']}'");
	...
	WHERE WO.AssignID=AG.EmployeeID AND WO.CreatedID=CR.EmployeeID AND WO.RequestorID=RQ.EmployeeID AND WO.AssetID=AE.AssetID AND WO.WorkTypeID=WT.WorkTypeID AND WO.WorkPriorityID=WP.WorkPriorityID AND WO.WorkStatusID=WS.WorkStatusID AND WO.WorkTradeID=WR.WorkTradeID AND WO.FailureCauseID=FC.FailureCauseID AND WO.Hidden='no' AND WO.DepartmentID ='{$_SESSION['section']}'");
	...
	DEFINE('QMANPOWER','SELECT EMP.EmployeeID ID, EMP.EmployeeNo No_ID, EMP.FirstName Name, WM.cost_hour Cost FROM work_order WO, employee EMP, work_order_manpower WM WHERE WO.WorkOrderNo=WM.WorkOrderNo AND EMP.EmployeeID=WM.EmployeeID');
	...
	DEFINE('NOTPLANPERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000002" OR WO.WorkStatusID="WS000003" OR WO.WorkStatusID="WS000004") ORDER BY State_modified_date, WorkOrderNo DESC');
	DEFINE('PERFORMED','SELECT WorkOrderNo WO, AssetDesc Asset, T.WorkTypeDesc Work_Type, E.FirstName Assign_To, S.WorkStatus Work_Status, ProblemDesc Description FROM work_order WO, asset A, work_status S, work_type T, employee E WHERE A.AssetID=WO.AssetID AND WO.WorkStatusID=S.WorkStatusID AND WO.AssignID=E.EmployeeID AND WO.WorkTypeID=T.WorkTypeID AND WO.Hidden="no" AND (WO.WorkStatusID="WS000007") AND YEAR(WO.ActDateEnd)=YEAR(CURRENT_DATE()) ORDER BY State_modified_date, WorkOrderNo DESC');
