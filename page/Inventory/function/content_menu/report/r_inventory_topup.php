<?php
	function report_itopup(){
		$content .= '<br/><div class="ade">'.TRINTOPUP.'</div>';
		//----- Buat Form Isian Berikut-----
		$name_field=array('Date Topup from','To','Vendor','Spare Part Name','State');
		$input_type=array(
						date_je(array('from','')),
						date_je(array('to','')),
						combo_je(array(COMBVENDOR,'vendor','vendor',350,'<option value="">-</option>','')),
						combo_je(array(COMBITEM,'item','item',350,'<option value="">-</option>','')),
						combo_je(array(COMBSTATE,'state','state',350,'<option value="">-</option>',''))
					);
		$signtofill = array('','','','','');
		$content .= create_form(array('',PATH_RINTOPUP.GEN,1,$name_field,$input_type,$signtofill)).js_topup();
		if(isset($_REQUEST['gen'])){
			if(!empty($_REQUEST['from']) && !empty($_REQUEST['to'])){
				$query = TOPUP.' AND IO.date_topup BETWEEN "'.$_REQUEST['from'].'" AND "'.$_REQUEST['to'].'" AND IO.vendor_id LIKE "%'.$_REQUEST['vendor'].'%" AND IO.item_id LIKE "%'.$_REQUEST['item'].'%" AND IO.state_journal_movement_id LIKE "%'.$_REQUEST['state'].'%" ORDER BY IO.date_topup DESC';
			}
			else{
				$query = TOPUP.' AND IO.vendor_id LIKE "%'.$_REQUEST['vendor'].'%" AND IO.item_id LIKE "%'.$_REQUEST['item'].'%" AND IO.state_journal_movement_id LIKE "%'.$_REQUEST['state'].'%" ORDER BY IO.date_topup DESC';
			}
			
			$info= excel_itopup(array($query,'Inventory Topup','0'));
		}
		return $info.$content;
	}
	
	function excel_itopup($data){
		$sql = $data[0];
		$name = $data[1];
		$sheet = $data[2];
		$content = ''; 
		...
		$objPHPExcel->getActiveSheet()->setCellValue('B1', 'Spare Part Name');
		$objPHPExcel->getActiveSheet()->setCellValue('C1', 'Unit');
		$objPHPExcel->getActiveSheet()->setCellValue('D1', 'Vendor');
