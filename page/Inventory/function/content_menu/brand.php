<?php
	function brand(){
		$content .= '<br/><div class="ade">'.TBRAND.'</div>';
			...
			if(isset($_REQUEST['add'])){
				$content = '<br/><div class="ade">'.TABRAND.'</div>';
				$content .= '<div class="toptext" align="center">'._USER_VIEW_._USER_INSERT_.'</div>';
				...
				$content .= create_form(array(FABRAND,PATH_BRAND.ADD.POST,1,$name_field,$input_type,$signtofill));
			...
			if(isset($_REQUEST['edit'])){ $info='';
				...
				$querydat = BRAND.' WHERE brand_id="'.$_REQUEST['rowid'].'"'; $result=mysql_exe_query(array($querydat,1)); $resultnow=mysql_exe_fetch_array(array($result,1));
				...
				$signtofill = array('<small id="fill" class="form-text text-muted">Please fill this field.</small>','');
				$content .= create_form(array(FAITEMCAT,PATH_BRAND.EDIT.'&rowid='.$_REQUEST['rowid'].POST,1,$name_field,$input_type,$signtofill));
